...
def test():
    ...
    # config_file = '/home/cslics04/cslics_ws/src/coral_spawn_imager/launch/camera_config_dev.json'
    # picam = PiCamera2Wrapper(config_file=config_file)
    # NOTE: camera needs to be made before writing to camera works!
    # NOTE: commented out temporarily to avoid circular import
    
    focuser = ArducamFocuser(10)
    print(f'get focus: {focuser.get(focuser.OPT_FOCUS)}')
    time.sleep(3)
    focuser.reset(focuser.OPT_FOCUS)
    time.sleep(3)
    focuser.set(focuser.OPT_FOCUS, 100)
    time.sleep(3)
    focuser.set(focuser.OPT_FOCUS, 500)
